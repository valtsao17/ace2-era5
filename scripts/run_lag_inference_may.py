#!/usr/bin/env python3
"""Run ACE2-ERA5 lag ensemble inference for the May 1 experiment (heat extremes).

For each year × member:
  - init: May 1 centre ± 12 × 6 h (25 members, Apr 28 – May 4)
  - n_forward_steps: 400  (100 days from Apr 28 centre → covers Aug 1 from all members)
  - output: <output_dir>/{year}/member_{idx:02d}/

Target dates extractable from the outputs: June 1, July 1, August 1.
...
def parse_args():
    ...
    p.add_argument("--output-dir", default=None,
                   help="Where to write run outputs (default: <root>/outputs/lag_may/runs)")
...
def resolve_paths(args):
    ...
    out_dir = Path(args.output_dir) if args.output_dir else root / "outputs/lag_may/runs"
    wrapper = (
        Path(args.wrapper) if args.wrapper
        else Path(__file__).parent / "07_run_ace2s_inference_with_ensemble_fix.py"
    )
...
def main():
    ...
    jobs = []
    for year in years:
        ic_path = ic_dir / f"ic_lag_{year}0501_25m.nc"
        if not ic_path.exists():
            print(f"ERROR: missing IC file for {year}: {ic_path}", file=sys.stderr)
            sys.exit(1)
        # Also verify forcing file exists for this year
        forcing_file = forcing_dir / f"forcing_{year}.nc"
        if not forcing_file.exists():
            print(f"ERROR: missing forcing file for {year}: {forcing_file}", file=sys.stderr)
            sys.exit(1)
