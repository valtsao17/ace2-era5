#!/usr/bin/env python3
"""One-panel RAW heat-extreme-frequency cluster skill comparison.

This is the frequency/risk analogue of raw_cluster_metric_panel.py.  It uses
the same four clustering/evaluation methods and the same two-row panel layout,
but clusters CONUS grid cells by their 37-year ACE2 JJA RAW heat-extreme
seasonal-frequency trajectories instead of raw JJA-mean Tmax trajectories.
...
Output:
  outputs/lag_may/cluster_analysis_sliding7d_conus/raw_frequency_metric_panel/
"""
...
# (feature building, borders, map utilities, domain means, REDCAP connectivity)
# resolves against the seasonal-frequency pipeline instead of raw-Tmax data.
panel.raw = src

FREQ_NC = PROJECT_ROOT / "outputs/lag_may/seasonal_jja_sliding7d/jja_seasonal_freqs.nc"
OUT_DIR = PROJECT_ROOT / "outputs/lag_may/cluster_analysis_sliding7d_conus/raw_frequency_metric_panel"
...
def load_conus_frequency():
    """CONUS ACE2/ERA5 JJA RAW heat-extreme seasonal-frequency fields."""
...
def main():
    ...
    print(
        f"CONUS RAW frequency fields: pred/obs={pred.shape}  features={features_full.shape}",
        flush=True,
    )
    ...
    summary = {
        "input": str(FREQ_NC),
        "clustering_field": "ACE2 JJA RAW heat-extreme seasonal frequency",
        "skill_pair": "ACE2 frequency vs ERA5 frequency",
        "metric": f"modified-Kendall z uses truncation k={panel.MODK_K}",
        "kendall_tau_colorbar": [-1.0, 1.0],
        "normalized_modkendall_colorbar": [-1.0, 1.0],
        "methods": {},
    }
    ...
    out_png = OUT_DIR / f"raw_frequency_cluster_metric_panel_{tag}{metric_suffix}.png"
    panel.render_panel(
        results,
        methods,
        lat,
        lon,
        out_png,
        suptitle="CONUS RAW heat-extreme frequency clusters: ACE2 vs ERA5 skill by cluster",
    )
    ...
    out_json = OUT_DIR / f"raw_frequency_cluster_metric_panel_summary{metric_suffix}.json"
