...
def plot_cluster_tau_map(tau_map_abs, labels_2d, tau_cl, tau_domain, lat, lon, title, out,
                         vmin=0.0, vmax=None, cmap=None, label_fontsize=7):
    """Per-cluster |τ| map: each cluster colored by its own τ, the numeric
    (signed) τ labeled at the cluster centroid, and the domain-mean τ called
    out in a corner annotation.
    ...
    a 2-cluster split), so a fixed 0-0.4 scale saturates and washes out
    low-k cluster maps.
    """
    cmap = cmap if cmap is not None else _HEAT_CMAP
    if vmax is None:
        finite = tau_map_abs[np.isfinite(tau_map_abs)]
        vmax = max(float(np.nanmax(finite)) * 1.05, 0.05) if finite.size else 0.4
    if len(lat) < 170:
        ...
        im = ax.imshow(tau_map_abs, origin="lower", extent=e, aspect="auto",
        ...
        data_r, lon_r = _roll_to_180(tau_map_abs, lon)
...
def plot_comparison(km_results, som_results, redcap_results,
    ...
    _three_panel_map(
        [np.abs(tau_gridpt_map), np.abs(km_opt["tau_map"]), np.abs(som_opt["tau_map"])],
    ...
    plot_cluster_tau_map(np.abs(km_opt["tau_map"]), km_labels_full, km_opt["tau_cl"],
    ...
    plot_cluster_tau_map(np.abs(som_opt["tau_map"]), som_labels_full, som_opt["tau_cl"],
    ...
    if rc_opt:
        ...
        plot_cluster_tau_map(np.abs(rc_opt["tau_map"]), rc_labels_full, rc_opt["tau_cl"],
        ...
        if km_match:
            ...
            plot_cluster_tau_map(np.abs(km_match["tau_map"]), km_match_labels, km_match["tau_cl"],
        ...
        plot_cluster_tau_map(np.abs(som_match["tau_map"]), som_match_labels, som_match["tau_cl"],
