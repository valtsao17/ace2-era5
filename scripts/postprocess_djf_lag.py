...
def plot_tau_conus(tau: np.ndarray, pval: np.ndarray,
    ...
    """CONUS+Alaska plot of |Kendall tau|, land only, stippled non-sig points."""
...
def plot_tau_global(tau: np.ndarray, lat: np.ndarray, lon_360: np.ndarray,
                    extreme: str, title: str, out_path: Path):
    """Simple global |tau| map on plain axes (no cartopy to avoid crashes)."""
    import cartopy.crs as ccrs
    import cartopy.feature as cfeature

    fig, ax = plt.subplots(figsize=(14, 6),
                           subplot_kw=dict(projection=ccrs.PlateCarree()))
    ax.pcolormesh(lon_360, lat, np.abs(tau), shading="auto",
                  cmap=_SKILL_CMAP, vmin=0.0, vmax=0.6,
...
def run_extreme(extreme: str, template: xr.DataArray, lat: np.ndarray,
    ...
    conus_title = (
        ...
        f"|  n={n_events}  |  window=±{WINDOW_DAYS}d LOO"
    )
